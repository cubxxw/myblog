 ...
 * @return void
 */
function ihafs_cmb2_render_select_multiple_field_type( $field, $escaped_value, $object_id, $object_type, $field_type_object ) {
	$select_multiple = '<select style="height:150px" class="widefat" multiple name="' . $field->args['_name'] . '[]" id="' . $field->args['_id'] . '"';
 ...
 */
function ihafs_cmb2_sanitize_select_multiple_callback( $override_value, $value ) {
	if ( is_array( $value ) ) {
...
add_filter( 'cmb2_sanitize_select_multiple', 'ihafs_cmb2_sanitize_select_multiple_callback', 10, 2 );