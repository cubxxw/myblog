...
(function ($) {
     ...
     * @type {String}
     */
    const searchInputSelector = '#wp-customizer-search-input';
    ...
    CustomizerSearchAdmin = {
        ...
        displayMatches: function (stringToMatch, controls) {
            ...
            html = matchArray.map(function(index, elem) {
                ...
                return `
                    <li id="accordion-section-${index.section}" class="accordion-section control-section control-section-default wp-customizer-search-results" aria-owns="sub-accordion-section-${index.section}" data-section="${index.section}">
            ...
            document.getElementById('search-results').innerHTML = `<ul id="wp-customizer-search-results">${html}</ul>`;
        ...
        _showSearchButtonToggle: function () {
            ...
            if ($('#customize-info #accordion-section-wp-customizer-search').length == 0) {
        ...
        _display_search_form: function () {
            ...
            if ($('#accordion-section-wp-customizer-search').hasClass('open')) {
                $('#accordion-section-wp-customizer-search').removeClass('open')
                $('#accordion-section-wp-customizer-search').slideUp('fast');
            } else {
                $('.customize-panel-description').removeClass('open');
                $('.customize-panel-description').slideUp('fast');

                $('#accordion-section-wp-customizer-search').addClass('open');
                $('#accordion-section-wp-customizer-search').slideDown('fast');
            }
        ...
        _clearSearch: function () {
            ...
            document.getElementById('wp-customizer-search-input').value = '';
